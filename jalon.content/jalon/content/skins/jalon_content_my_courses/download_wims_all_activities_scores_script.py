# -*- coding: utf-8 -*-
## Controller Python Script "download_wims_all_activities_scores_script.py"
...
# [TODO] : à mutualiser avec download_wims_scores_script.py ?
...
if course_object.isAuteurs(user_id):
    ...
    if "utilisateur" in request:
        ...
        utilisateur = "--aucun--"
        message = _(u"Vous n'avez selectionné aucun utilisateur.")
    ...
    if resultat["status"] == "OK":
        # message = _(u"Les notes des activités WIMS du cours « <strong>%s</strong> » ont bien été téléchargées ." % (titre_cours))
        # context.plone_utils.addPortalMessage(message, "info")
    ...
    context.plone_utils.addPortalMessage(message, "alert")
    context.envoyerMailErreur({"objet"  : _(u"Accès non autorisé à '%s'" % "getscores_wims"),
                               "message": _("<p>Suspicion de fraude de l'utilisateur <strong>%s</strong></p><div>%s</div>" % (user_id, request))})
