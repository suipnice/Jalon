...
class AnzCASClient( BasePlugin, Cacheable ):
    ...
    def _setCookie(self, cookie, response):
        ...
        options = dict(path="/", http_only=True)
        response.setCookie("__accas", cookie, **options)

    security.declarePrivate( 'extractCredentials' )
    def extractCredentials( self, request ):
        #print "CAS : extractCredentials"
        ...
        if not request.cookies.get("__accas"):
           #print "not cookie __accas"
           ...
           if len(request.form.keys()) > 1:
               ...
               for key in request.form.keys():
                   if key != "ticket":
                      param.append("%s=%s" % (key, request.form[key]))
               service = "".join([service, quote("?%s" % "&".join(param))])
               #print service
           login = self.validateServiceTicket(service, ticket).getPrincipal().getId()
           #print "login"
           #print login
           cookie = self._setupSession(login, request.RESPONSE)
           #print "cookie __accas added"
           #print cookie
        else:
           #print "has cookie __accas"
           cookie = binascii.a2b_base64(request.cookies.get("__accas"))
           #print cookie
           data = tktauth.validateTicket(getUtility(IKeyManager).secret(), cookie)
           #print "cookie translate"
           #print data
           login = data[1].strip()
        creds["login"] = login.strip()
        #print "credentials"
        #print creds
        return creds
    ...
    def authenticateCredentials( self, credentials ):
        #print "CAS : authenticateCredentials"
        if credentials['extractor'] != self.getId(): return None
    ...
    def challenge( self, request, response, **kw ):
        ...
        response.expireCookie("__accas", path="/")
